...
export const AppointmentSlotsController = {
  create: async (req: Request, res: Response): Promise<void> => {
    try {
      const { startTime, endTime, appointmentDuration, restTime, medicalEstablishmentId } = req.body;
      ...
      if (!startTime || !endTime || typeof startTime !== 'string' || typeof endTime !== 'string') {
        res.status(400).json({ 
          message: 'Start time and end time are required and must be strings in HH:MM format' 
        });
        return;
      }
