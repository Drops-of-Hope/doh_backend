...
export const BloodController = {
  ...
  listNearingExpiryUnits: async (req: Request, res: Response): Promise<void> => {
    try {
      const { inventory_id } = req.body ?? {};
      ...
      const { items, totalAvailableUnits, count } = await BloodService.listNearingExpiryUnits(
        inventory_id
      );

      if (count === 0) {
        res.status(200).json({
          message: "No safe and available blood units nearing expiry found for this inventory.",
          available_units: 0,
          count: 0,
          data: [],
        });
  ...
  listUnitsByInventory: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { items, totalAvailableUnits, count } = await BloodService.listUnitsByInventory(
        inventory_id
      );

      if (count === 0) {
        res.status(200).json({
          message: "No safe and available blood units found for this inventory.",
          available_units: 0,
          count: 0,
          data: [],
        });
